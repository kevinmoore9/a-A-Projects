...
def costars(name)
  ...
  Actor.select(:name)
       .joins(:castings)
       .where('castings.movie_id IN (?)', Casting.select(:movie_id).joins(:actor).where('actors.name = ?', name).pluck(:movie_id))
       .pluck(:name)
      #  .where('actors.name != ?', name)

  # Actor.select(:name)
  #      .joins(:castings).select(:movie_id).joins(:actor).where('actors.name = ?', name))
  #      .where('name != ?', name)
